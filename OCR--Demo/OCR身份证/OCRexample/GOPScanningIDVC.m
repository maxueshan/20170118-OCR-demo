...
@interface GOPScanningIDVC() <UITextFieldDelegate, UITextViewDelegate>  {
...
@implementation GOPScanningIDVC
@synthesize IDInfo;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //自动启用正面
    [self launchCameraViewWithFront:YES];
...
//        bShouldFront = NO;
//    }
    EXOCRIDCardRecoManager *manager = [EXOCRIDCardRecoManager sharedManager:nil];
    //扫描页的相关设置
    [manager setDisplayLogo:NO];     //是否显示logo
    [manager setEnablePhotoRec:NO]; //是否开启本地相册识别
    [manager setScanFrameColorRGB:0x213 andAlpha:0.3];//扫描页设置，扫描框颜色
